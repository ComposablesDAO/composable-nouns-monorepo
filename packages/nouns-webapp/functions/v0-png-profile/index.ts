...
const handler: Handler = async (event, context) => {
  ...
  const walletAddress = queryParams['address'];
  
  const conn = connect(configIndexer);
  const results = await conn.execute('SELECT walletAddress, bannerImage FROM profiles WHERE walletAddress = ?', [walletAddress]);
